...
class Block {
  ...
  get dataFrame(): Buffer {
    let dataOffset = this.disk.BLOCK_SIZE * this.id + DiskInterface.INDICATOR_SIZE + this.disk.HEADER_SIZE;
  ...
  get next(): Block | null {
    let pointerOffset =
      this.disk.BLOCK_SIZE * this.id + this.disk.HEADER_SIZE + DiskInterface.INDICATOR_SIZE + this.disk.BLOCK_DATA_SIZE;
